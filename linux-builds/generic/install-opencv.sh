...
cmake -DBUILD_opencv_videoio=OFF -DBUILD_opencv_video=OFF -DWITH_CUDA=OFF -DOPENCV_EXTRA_MODULES_PATH=$OPENCV_CONTRIB_MODULES -DCMAKE_BUILD_TYPE=$DEPENDENCY_CMAKE_BUILD_TYPE -DBUILD_SHARED_LIBS=$SHARED_BUILD_OME -DCMAKE_INSTALL_PREFIX=$INSTALL_PREFIX -G "Unix Makefiles" .. || { echo 'Build configuration failed' ; exit 1; }
